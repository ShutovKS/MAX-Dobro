...
const SettingsSection: React.FC<{ title: string; children: React.ReactNode }> =
  ({ title, children }) => (
    <section>
      <h2 className="px-6 pb-2 text-sm font-semibold text-[rgb(12,13,14,0.52)] uppercase tracking-wider">
        {title}
      </h2>
  ...
  >
    <div className="flex items-center space-x-4">
      ...
      >
        <Icon
          className={`w-5 h-5 ${
            isDestructive ? 'text-[#FF303C]' : 'text-[#007AFF]'
          }`}
        />
      </div>
      <span
        className={`font-semibold ${
          isDestructive ? 'text-[#FF303C]' : 'text-[#0C0D0E]'
        }`}
      >
        {label}
      </span>
    ...
    <>
      <div className="w-full h-screen font-sans antialiased bg-[#F0F0F0] flex flex-col">
        <header className="flex-shrink-0 p-6 pb-4 bg-white/80 backdrop-blur-sm border-b border-gray-200 flex items-center">
          ...
          <h1 className="text-2xl font-bold text-[#0C0D0E] mx-auto">
            Настройки
          </h1>
        ...
        <main className="flex-grow overflow-y-auto pt-8 space-y-8 pb-8">
          ...
          <SettingsSection title="Уведомления">
            <div className="p-4 flex items-center space-x-4">
              ...
              <span className="font-semibold text-[#0C0D0E]">
                Push-уведомления
              </span>
            </div>
            <div className="pl-16 pr-4 py-2 border-t border-gray-100">
              <ToggleRow
                label="Новые события от подписок"
                enabled={notifications.newEvents}
                onChange={() => handleToggle('newEvents')}
              />
            </div>
            <div className="pl-16 pr-4 py-2 border-t border-gray-100">
              <ToggleRow
                label="Напоминания о моих событиях"
