...
async function main() {
  ...
  const courseIntro = await prisma.course.create({
    data: { id: 2, title: "Введение в волонтерство", description: "Узнай все о том, как стать волонтером и выбрать направление по душе." }
  });
  const courseEco = await prisma.course.create({
    data: { id: 3, title: "Эко-активист", description: "Как помогать планете каждый день.", icon: 'leaf', lessons: { create: { title: 'Тест по раздельному сбору', content: 'Основы сортировки мусора.', questions: { create: [
      { question: 'Что из перечисленного НЕЛЬЗЯ сдавать в макулатуру?', answers: { create: [{ answer: 'Старая газета', isCorrect: false }, { answer: 'Картонная коробка', isCorrect: false }, { answer: 'Бумажный чек из магазина', isCorrect: true }, { answer: 'Глянцевый журнал', isCorrect: false }] } },
      { question: 'Какую маркировку пластика чаще всего принимают на переработку?', answers: { create: [{ answer: '1 (PET) и 2 (HDPE)', isCorrect: true }, { answer: '3 (PVC) и 7 (OTHER)', isCorrect: false }, { answer: '6 (PS)', isCorrect: false }] } },
    ] } } } }
  });
  await prisma.userCertificate.create({ data: { userId: mainUser.id, courseId: courseIntro.id } });
  console.log('✅ Courses with lessons created.');
