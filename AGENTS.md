...
- `bot-max/` and `bot-telegram/` hold messenger entry bots that deep-link into the mini-app.
- `docs/` holds GRACE artifacts: requirements, technology, development plan, verification plan, knowledge graph, and operational packets.
...
## Keywords
volunteer, charity, karma, events, courses, stories, organizer, NestJS, React, Prisma, Supabase, MAX, Telegram

## Annotation
Volunteer and charity platform that gamifies good deeds. NestJS API, React mini-app, MAX and Telegram bots, PostgreSQL via Prisma/Supabase.
...
- Find implementation files tied to graph context: search `LINKS:` plus the module ID in `backend/`, `frontend/`, `bot-max/`, `bot-telegram/`
...
Adapt comment syntax to the file format: `//` in TypeScript, `{/* */}` inside TSX JSX, `///` in Prisma schema.
...
backend/src/            - NestJS API with GRACE markup
frontend/src/           - React mini-app with GRACE markup
bot-max/src/            - MAX bot entry
bot-telegram/src/       - Telegram bot entry
backend/test/           - e2e tests with GRACE-aware evidence where appropriate
```
...
- Backend startup and module wiring: `backend/src/main.ts` (M-BACKEND-BOOTSTRAP), `backend/src/app.module.ts` (M-BACKEND-APP)
- API and auth boundaries: `frontend/src/lib/api.ts` (M-FRONTEND-API), `frontend/src/lib/auth.ts` (M-FRONTEND-AUTH)
- Route assembly and auth entry: `frontend/src/app/page.tsx` (M-FRONTEND-APP), `frontend/src/app/auth/page.tsx` (M-FRONTEND-SCREENS)
- Schema and seed boundaries: `backend/prisma/schema.prisma` (M-SCHEMA), `backend/prisma/seed.ts` (M-SEED)
- Knowledge graph: `docs/knowledge-graph.xml`

<guardrails>

- Do not change runtime behavior when adding GRACE markup.
- Govern every important source file with MODULE_CONTRACT; add START_CONTRACT for exported functions and START_BLOCK around ~500-token logical slices.
- Tiny DTO/entity/icon files get a contract and map; they do not need dense inner blocks.
